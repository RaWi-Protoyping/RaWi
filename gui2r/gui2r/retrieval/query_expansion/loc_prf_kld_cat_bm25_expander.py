...
class LocalPrfKldCatBM25ExpanderRanker(Ranker):
    ...
    def get_top_expansion_cands_per_cat(self, probs_t_dr: Dict[Text, Dict[Text, float]],
                                ...
                                ensure_unique: Optional[bool] = True) -> List[Text]:
    ...
    def get_top_expansion_cands(probs_t_dr: Dict[Text, float], probs_t_dc: Dict[Text, float],
        ...
        for word, prob_t_dr in probs_t_dr.items():
            if word not in rm_tokens:
                scores_kld[word] = LocalPrfKldCatBM25ExpanderRanker.\
    ...
    def load(conf: Configuration, force: Optional[bool] = False,
             persist: Optional[bool] = True) -> "LocalPrfKldCatBM25ExpanderRanker":
        dataset = LocalPrfKldCatBM25ExpanderRanker.extractor.load_dataset(conf=conf)
        ...
        logging.info('LocPrfKldCatRanker: Using Text Segments: {}'.format(text_segments))
        ...
        counter, probs = LocalPrfKldCatBM25ExpanderRanker.create_counter(dataset, text_segments=text_segments)
        return LocalPrfKldCatBM25ExpanderRanker(conf=conf, internal_ranker=internal_ranker,
    ...
    def get_name(self):
        return Ranker.R_PRF_KLD_CAT_BM25
